...
ESQUEMA JSON REQUERIDO:
{
  ...
  "gunter_note": "1-2 frases"
    ...
    async function generate(analysisId, { projectInfo, transcription, environment }) {
        ...
        const useProxy = cfg.USE_PROXY !== undefined ? cfg.USE_PROXY : true;
        const url = useProxy ? cfg.PROXY_CHAT_URL : (cfg.OPENAI_CHAT_URL || 'https://api.openai.com/v1/chat/completions');
        const headers = { 'Content-Type': 'application/json' };
        if (!useProxy && cfg.OPENAI_API_KEY) headers['Authorization'] = `Bearer ${cfg.OPENAI_API_KEY}`;
