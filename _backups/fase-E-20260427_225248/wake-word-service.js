...
(function () {
    ...
    if (!SR) {
        window.GunterWakeWord = {
            supported: false,
            isActive: () => false,
            start: () => { throw new Error('SpeechRecognition no soportado'); },
            stop: () => {},
            refresh: () => {},
            requestPermission: async () => 'unsupported',
            getState: () => ({ active: false, mode: 'off', supported: false }),
            getLastHeard: () => ''
        };
        return;
    }
    ...
    function enterQueryMode(preText = '') {
        ...
        try {
            ...
            if (window.GunterVoice?.speak) {
                window.GunterVoice.speak(feedback, { context: 'wake-word-response', force: true, volume: 0.85 });
    ...
    async function handleQuery(transcript) {
        ...
        if (window.GunterPipeline?.handleUserInput) {
            try {
                ...
                // Siempre hablar la respuesta cuando viene de wake word
                if (window.GunterVoice?.speak) {
                    window.GunterVoice.speak(reply, { context: 'chat', force: true });
                }
                ensureAssistantOpen();
